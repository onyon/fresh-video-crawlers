...
export default class ParamountPlus extends Crawler {
  ...
  async crawl() {
    ...
    this.local = {
      link: {
        index: `https://www.paramountplus.com/movies_xhr/all/`,
      }
    }
    ...
    for(let i = 0; true; i++) {
      ...
      try {
        const n = index.data[0].result.data.length;
      } catch(e) {
        break;
      }

      for (const e of index.data[0].result.data) {

        if(e.title.includes("(Trailer)"))
          continue;

        const movieHTML = await this.getHTML(`https://www.paramountplus.com${e.href}`);
        const regex = /player\.apiMetadata = (.+);/gm;
        const movieMeta = JSON.parse(regex.exec(movieHTML)[1]);

        let movie = {
          Title: e.title,
          Description: movieMeta.description,
          Link: `https://www.paramountplus.com${e.href}`,
          Year: moment(movieMeta["_airDateISO"]).format("YYYY"),
          ReleaseDate: moment(movieMeta["_airDateISO"]).format("YYYY-MM-DD"),
          RetrieveImage: async function(self) {

            return await self.getBuffer(movieMeta.thumbnail);
